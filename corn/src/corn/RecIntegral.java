...
public class RecIntegral implements Serializable{
    ...
    {
        double result = 0.0;
        double integralPart = (maxValue - minValue) / 2;
        
        LinkedList<Integral> threads = new LinkedList<>();
        
        long startTime = System.currentTimeMillis();
        
        for (double start = minValue; start + integralPart < maxValue; start += integralPart)
        {
            threads.add(new Integral(start, start+integralPart, stepValue));
            threads.getLast().start();
        }
        
        for (Integral thread : threads)
        {
            try 
            {
                thread.join();
            ...
            result += thread.getResultValue();
            thread.isAlive();
        }
        
        long endTime = System.currentTimeMillis();
        System.out.println("Estimation time: " + (endTime - startTime) + " ms");
